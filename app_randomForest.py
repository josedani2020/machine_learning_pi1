...
def main():
    ...
    print(f"Tiempo de modelo de machine learning: {ml_model_end - ml_model_start:.2f} segundos")
    ...
    with connection.cursor() as cursor:
        for index, row in df_ventas.loc[X_test.index].iterrows():
            cursor.execute("INSERT INTO RESULTADOS_CLASIFICACION_RF (COD_DISTRIBUIDORA, COD_PRODUCTO, Predicted_Homologado, Fecha_Registro) VALUES (?, ?, ?, ?)", row['COD_DISTRIBUIDORA'], row['COD_PRODUCTO'], row['Predicted_Homologado'], now)
        connection.commit()
